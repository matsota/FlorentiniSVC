...
class StoreStaticticViewController: UIViewController {
    ...
    @IBAction private func receiptsOverSomePriceTapped(_ sender: UIButton) {
        self.present(UIAlertController.setNewInteger(message: "Введите сумму покупки, и вам покажет количество чеков, сумма которых БОЛЬШЕ введённой цифры", confirm: { (number) in
    ...
    @IBAction private func receiptsLessSomePriceTapped(_ sender: UIButton) {
        self.present(UIAlertController.setNewInteger(message: "Введите сумму покупки, и вам покажет количество чеков, сумма которых МЕНЬШЕ введённой цифры", confirm: { (number) in
